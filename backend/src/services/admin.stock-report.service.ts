import { StockAction } from "../generated/prisma/client";
...
export async function getStockReport(params: StockReportParams) {
  ...
  for (const stock of stocks) {
    ...
    if (stock.product.category?.name) categoryName = stock.product.category.name;
    stockRows.push({
      id: stock.id,
  ...
  for (const row of stockRows) stockIds.push(row.id);
  ...
  const journalsByStock = new Map<string, any[]>();
  ...
  const perStockSummary: any[] = [];
  for (const stock of stockRows) {
    const entries = journalsByStock.get(stock.id) || [];
    const entriesInMonth: typeof entries = [];
    let totalIn = 0;
    let totalOut = 0;
    let netAfterMonth = 0;
